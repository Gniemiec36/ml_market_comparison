...
Pipeline:
Data acquisition — download OHLC data for a given ticker, period, and interval via yfinance
Returns — compute simple and log returns from closing price
Feature engineering — rolling mean/std of log returns, and RSI (Relative Strength Index), all computed over a configurable window
Target definition — binary label: whether the next period's simple return is positive
Train/test split — chronological (non-shuffled) 80/20 split to avoid lookahead bias
Normalisation — StandardScaler fit on training data only, applied to both sets
Modelling — four independent approaches trained and evaluated on identical data:
...
Evaluation — accuracy and classification report for each model; trained models are serialised with joblib
